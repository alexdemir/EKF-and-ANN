...
class AnnTrainer(Node):
    def __init__(self):
        ...
        self.declare_parameter("pause_training_on_slip", True)
        self.declare_parameter("slip_training_error_threshold", 1.60)
        self.declare_parameter("slip_training_min_odom_speed", 0.10)
        self.declare_parameter("slip_gate_max_integrated_imu_speed", 2.0)
        self.declare_parameter("slip_gate_imu_velocity_decay_tau", 12.0)
        self.declare_parameter("slip_gate_odom_correction_tau", 4.0)
        self.declare_parameter("slip_gate_odom_correction_gate", 1.2)
        ...
        self.pause_training_on_slip = bool(self.get_parameter("pause_training_on_slip").value)
        self.slip_training_error_threshold = float(
            self.get_parameter("slip_training_error_threshold").value
        )
        self.slip_training_min_odom_speed = float(
            self.get_parameter("slip_training_min_odom_speed").value
        )
        self.slip_gate_max_integrated_imu_speed = float(
            self.get_parameter("slip_gate_max_integrated_imu_speed").value
        )
        self.slip_gate_imu_velocity_decay_tau = float(
            self.get_parameter("slip_gate_imu_velocity_decay_tau").value
        )
        self.slip_gate_odom_correction_tau = float(
            self.get_parameter("slip_gate_odom_correction_tau").value
        )
        self.slip_gate_odom_correction_gate = float(
            self.get_parameter("slip_gate_odom_correction_gate").value
        )
        ...
        self.slip_training_pause_announced = False
        ...
        self.slip_gate_imu_body_velocity = np.zeros(2)
        ...
        self.get_logger().info(
            ...
            f"pause_training_on_slip={self.pause_training_on_slip}, "
            f"slip_training_error_threshold={self.slip_training_error_threshold:.3f}"
        )
    ...
    def imu_callback(self, msg):
        ...
        if self.last_imu_stamp is not None:
            ...
            if 0.0 < dt < 0.5:
                ...
                vx, vy = self.slip_gate_imu_body_velocity
                dvx = msg.linear_acceleration.x + yaw_rate * vy
                dvy = msg.linear_acceleration.y - yaw_rate * vx
                self.slip_gate_imu_body_velocity += np.array([dvx, dvy]) * dt
                if self.slip_gate_imu_velocity_decay_tau > 0.0:
                    decay = math.exp(-dt / self.slip_gate_imu_velocity_decay_tau)
                    self.slip_gate_imu_body_velocity *= decay
                self.correct_slip_gate_imu_velocity_with_odom(dt)
                slip_gate_speed = float(np.linalg.norm(self.slip_gate_imu_body_velocity))
                if (
                    self.slip_gate_max_integrated_imu_speed > 0.0
                    and slip_gate_speed > self.slip_gate_max_integrated_imu_speed
                ):
                    self.slip_gate_imu_body_velocity *= (
                        self.slip_gate_max_integrated_imu_speed / slip_gate_speed
                    )
    ...
    def correct_slip_gate_imu_velocity_with_odom(self, dt):
        if self.latest_odom is None:
            return
        if self.slip_gate_odom_correction_tau <= 0.0:
            return
        ...
        disagreement = float(np.linalg.norm(
            self.slip_gate_imu_body_velocity - odom_body_velocity
        ))
        gate = max(self.slip_gate_odom_correction_gate, 1e-6)
        correction_gate = math.exp(-0.5 * (disagreement / gate) ** 2)
        correction_alpha = 1.0 - math.exp(-dt / self.slip_gate_odom_correction_tau)
        correction_alpha *= correction_gate
        self.slip_gate_imu_body_velocity += correction_alpha * (
            odom_body_velocity - self.slip_gate_imu_body_velocity
        )
    ...
    def target_callback(self, msg):
        ...
        if self.should_pause_training_for_slip():
            self.announce_slip_training_paused()
    ...
    def should_pause_training_for_slip(self):
        if not self.pause_training_on_slip:
            return False
        if self.latest_odom is None:
            return False

        odom_speed = math.hypot(
            self.latest_odom.twist.twist.linear.x,
            self.latest_odom.twist.twist.linear.y,
        )
        if odom_speed < self.slip_training_min_odom_speed:
            return False

        imu_speed = float(np.linalg.norm(self.slip_gate_imu_body_velocity))
        ...
        return velocity_error >= self.slip_training_error_threshold

    def announce_slip_training_paused(self):
        if self.slip_training_pause_announced:
            return
        odom_speed = math.hypot(
            self.latest_odom.twist.twist.linear.x,
            self.latest_odom.twist.twist.linear.y,
        ...
        self.get_logger().warn(
            f"{ANSI_YELLOW}===== ANN TRAINING PAUSED: SLIP-LIKE MOTION ===== "
            f"GPS is available, but velocity_error={velocity_error:.3f} m/s "
            f"(imu_speed={imu_speed:.3f}, odom_speed={odom_speed:.3f}) is above "
            f"threshold={self.slip_training_error_threshold:.3f}. "
            f"Model update skipped to avoid contaminating training data. ====={ANSI_RESET}"
        )
        self.slip_training_pause_announced = True
    ...
    def open_dataset_writer(self):
        ...
        try:
            ...
            if os.path.exists(self.dataset_path) and os.path.getsize(self.dataset_path) > 0:
                root, ext = os.path.splitext(self.dataset_path)
                backup_path = f"{root}.previous_{int(time.time())}{ext}"
                os.replace(self.dataset_path, backup_path)
                self.get_logger().warn(
                    f"{ANSI_YELLOW}Existing ANN dataset was not overwritten. "
                    f"Moved previous dataset to: {backup_path}{ANSI_RESET}"
                )
    ...
    def parse_forced_dropout_windows(self, text):
        ...
        for item in text.split(","):
            ...
            try:
                ...
                self.get_logger().warning(
                    f"Could not parse forced_dropout_windows entry: {item}"
                )
        return windows
