...
class EkfOutputLogger(Node):
    def __init__(self):
        ...
        self.declare_parameter("truth_topic", "/bumperbot_controller/odom")
        self.declare_parameter("reference_source", "reference")
        ...
        self.reference_source = str(self.get_parameter("reference_source").value).strip().lower()
        if self.reference_source not in ("reference", "gps", "truth"):
            self.get_logger().warn(
                "reference_source must be reference, gps, or truth; using reference"
            )
        ...
        self.truth_fallback_warned = False
        ...
        self.create_subscription(Odometry, self.truth_topic, self.truth_callback, 20)
        ...
        self.get_logger().info(
            "EKF output logging enabled: %s, reference_source=%s, truth_topic=%s"
            % (self.log_path, self.reference_source, self.truth_topic)
        )
    ...
    def ekf_callback(self, msg):
        ...
        truth_xy = self.xy_from_msg(self.latest_truth)
        eval_reference_xy, eval_reference_source = self.evaluation_reference(
            reference_xy,
            gps_xy,
            truth_xy,
        )
        ...
        self.log_writer.writerow([
            ...
            *self.format_xy(truth_xy),
            eval_reference_source,
            *self.format_xy(eval_reference_xy),
            ...
            *self.format_error_components(ekf_xy, eval_reference_xy),
            *self.format_error_components(pseudo_xy, eval_reference_xy),
            *self.format_error_components(odom_xy, eval_reference_xy),
            *self.format_error_components(ann_xy, eval_reference_xy),
            *self.format_error_components(kf_imu_xy, eval_reference_xy),
            *self.format_error_components(complementary_xy, eval_reference_xy),
            *self.format_error_components(final_xy, eval_reference_xy),
        ])
        self.log_file.flush()
    ...
    def open_log_writer(self):
        ...
        self.log_writer.writerow([
            ...
            "evaluation_reference_x",
            "evaluation_reference_y",
            "odom_x",
            "odom_y",
            "ann_x",
            "ann_y",
    ...
    def evaluation_reference(self, reference_xy, gps_xy, truth_xy):
        if self.reference_source == "truth":
            if all(math.isfinite(value) for value in truth_xy):
                return truth_xy, "truth"
            if not self.truth_fallback_warned:
                self.get_logger().warn(
                    "truth reference requested but not available; falling back to reference_topic"
                )
                self.truth_fallback_warned = True
            return reference_xy, "reference_fallback"
        if self.reference_source == "gps":
            if all(math.isfinite(value) for value in gps_xy):
                return gps_xy, "gps"
            return reference_xy, "reference_fallback"
        return reference_xy, "reference"
