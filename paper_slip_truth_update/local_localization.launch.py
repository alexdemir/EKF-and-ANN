...
def generate_launch_description():
    ...
    ann_forced_dropout_windows = LaunchConfiguration("ann_forced_dropout_windows")
    ekf_logger_reference_source = LaunchConfiguration("ekf_logger_reference_source")
    ekf_logger_truth_topic = LaunchConfiguration("ekf_logger_truth_topic")
    ...
    ann_trainer = Node(
        ...
        parameters=[
            ann_trainer_config,
            {
                ...
                "forced_dropout_windows": ann_forced_dropout_windows,
            },
            {"use_sim_time": True}
        ],
    ...
    ekf_output_logger = Node(
        ...
        parameters=[
            ...
            {
                "reference_source": ekf_logger_reference_source,
                "truth_topic": ekf_logger_truth_topic,
            },
            {"use_sim_time": True}
        ],
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            ...
            default_value="",
            description=(
                "Comma-separated forced GPS dropout windows as start:duration pairs, "
                "for example '45:20,90:20,130:20'. Overrides the single after/duration window."
            ),
        ),
        DeclareLaunchArgument(
            "ekf_logger_reference_source",
            default_value="truth",
            description="Evaluation reference for ekf_output_log.csv: truth, gps, or reference.",
        ),
        DeclareLaunchArgument(
            "ekf_logger_truth_topic",
            default_value="/bumperbot_controller/odom",
            description="Truth-like clean simulated odometry topic used for paper-style evaluation.",
        ),
        imu_republisher,
        ekf_local,
