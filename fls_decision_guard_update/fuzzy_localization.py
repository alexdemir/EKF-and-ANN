...
class FuzzyLocalization(Node):
    def __init__(self):
        ...
        self.declare_parameter("slip_latch_enabled", True)
        self.declare_parameter("slip_latch_trigger_error", 0.20)
        ...
        self.slip_latch_enabled = bool(
            self.get_parameter("slip_latch_enabled").value
        )
        self.slip_latch_trigger_error = float(
            self.get_parameter("slip_latch_trigger_error").value
        )
        self.slip_latch_release_error = float(
            self.get_parameter("slip_latch_release_error").value
        )
        self.slip_latch_hold_sec = float(
            self.get_parameter("slip_latch_hold_sec").value
        )
        ...
        self.severe_slip_disagreement = float(
            self.get_parameter("severe_slip_disagreement").value
        )
        self.severe_slip_disagreement_rate = float(
            self.get_parameter("severe_slip_disagreement_rate").value
        )
        self.severe_slip_min_fuzzy_error = float(
            self.get_parameter("severe_slip_min_fuzzy_error").value
        )
        ...
        self.slip_latch_until = -math.inf
        self.slip_latch_active = False
    ...
    def timer_callback(self):
        ...
        ann_kf2_disagreement_rate = self.update_disagreement_rate(
            ann_kf2_disagreement
        )
        slip_latch_alpha, severe_slip_boost = self.slip_latch_alpha(
            fuzzy_input_error,
            ann_kf2_disagreement,
    ...
    def update_disagreement_rate(self, ann_kf2_disagreement):
        now = self.clock_seconds()
        rate = 0.0
        if (
            self.last_disagreement_stamp is not None
            and self.last_ann_kf2_disagreement is not None
        ):
            dt = now - self.last_disagreement_stamp
            if 0.0 < dt < 1.0:
                rate = (ann_kf2_disagreement - self.last_ann_kf2_disagreement) / dt
        self.last_disagreement_stamp = now
        self.last_ann_kf2_disagreement = ann_kf2_disagreement
        return rate

    def slip_latch_alpha(
        self,
        fuzzy_input_error,
        ann_kf2_disagreement,
        ann_kf2_disagreement_rate,
    ):
        if not self.slip_latch_enabled:
            self.slip_latch_active = False
            self.severe_slip_boost_active = False
            return 0.0, 0.0
        if not math.isfinite(fuzzy_input_error):
            return 0.0, 0.0

        now = self.clock_seconds()
        if fuzzy_input_error >= self.slip_latch_trigger_error:
            self.slip_latch_until = max(
                self.slip_latch_until,
                now + max(self.slip_latch_hold_sec, 0.0),
            )
            if not self.slip_latch_active:
                self.get_logger().warn(
                    "\033[1;31m===== FLS SLIP LATCH ACTIVE ===== "
                    f"fuzzy_error={fuzzy_input_error:.3f}, "
                    f"hold={self.slip_latch_hold_sec:.1f}s, "
                    f"ann_weight={self.slip_latch_ann_weight:.2f}. =====\033[0m"
                )
            self.slip_latch_active = True
        elif (
            self.slip_latch_active
            and now > self.slip_latch_until
            and fuzzy_input_error <= self.slip_latch_release_error
        ):
            self.get_logger().warn(
                "\033[1;32m===== FLS SLIP LATCH RELEASED ===== "
                f"fuzzy_error={fuzzy_input_error:.3f}. =====\033[0m"
            )
            self.slip_latch_active = False

        if self.slip_latch_active and now <= self.slip_latch_until:
            latch_alpha = self.clamp01(self.slip_latch_ann_weight)
            severe_alpha = self.severe_slip_alpha(
                fuzzy_input_error,
                ann_kf2_disagreement,
                ann_kf2_disagreement_rate,
            )
            return latch_alpha, severe_alpha

        if self.severe_slip_boost_active:
            self.get_logger().warn(
                "\033[1;32m===== FLS SEVERE SLIP BOOST RELEASED ===== "
                f"disagreement={ann_kf2_disagreement:.3f} m. =====\033[0m"
            )
        self.severe_slip_boost_active = False
        return 0.0, 0.0

    def severe_slip_alpha(
        self,
        fuzzy_input_error,
        ann_kf2_disagreement,
        ann_kf2_disagreement_rate,
    ):
        if not self.severe_slip_boost_enabled:
            self.severe_slip_boost_active = False
            return 0.0
        if fuzzy_input_error < self.severe_slip_min_fuzzy_error:
            self.severe_slip_boost_active = False
            return 0.0

        disagreement_large = (
            ann_kf2_disagreement >= self.severe_slip_disagreement
        )
        disagreement_growing = (
            ann_kf2_disagreement_rate >= self.severe_slip_disagreement_rate
        )
        severe = disagreement_large or disagreement_growing
        if not severe:
            if self.severe_slip_boost_active:
                self.get_logger().warn(
                    "\033[1;32m===== FLS SEVERE SLIP BOOST RELEASED ===== "
                    f"disagreement={ann_kf2_disagreement:.3f} m, "
                    f"rate={ann_kf2_disagreement_rate:.3f} m/s. =====\033[0m"
                )
            self.severe_slip_boost_active = False
            return 0.0

        if not self.severe_slip_boost_active:
            self.get_logger().warn(
                "\033[1;31m===== FLS SEVERE SLIP BOOST ACTIVE ===== "
                f"recent velocity slip plus ANN-KF2 separation detected: "
                f"disagreement={ann_kf2_disagreement:.3f} m, "
                f"rate={ann_kf2_disagreement_rate:.3f} m/s, "
                f"ann_weight={self.severe_slip_ann_weight:.2f}. =====\033[0m"
            )
        self.severe_slip_boost_active = True
        return self.clamp01(self.severe_slip_ann_weight)
    ...
    def write_log_row(
        ...
        self.log_writer.writerow([
            ...
            f"{alpha_kf2:.9f}",
            f"{alpha_target:.9f}",
            f"{slip_latch_alpha:.9f}",
            str(bool(slip_latch_active)),
            f"{severe_slip_boost:.9f}",
            str(bool(severe_slip_boost_active)),
            f"{disagreement_boost:.9f}",
            f"{ann_kf2_disagreement:.9f}",
            f"{ann_kf2_disagreement_rate:.9f}",
